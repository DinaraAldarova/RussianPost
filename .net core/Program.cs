...
namespace post_service
{
    class Program
    {
        ...
        public static List<Barcode> GetBarcodes()
        {
            ...
            Log.Information(string.Format("Получено {0} ШПИ", barcodes.Count));
            return barcodes;
        }
        ...
        public static List<Ticket> GetTickets(List<Barcode> barcodes, AuthInfo auth)
        {
            ...
            Log.Information(string.Format("Получено {0} билетов", tickets.Count));
            return tickets;
        }
        ...
        public static List<Item> GetItems(List<Ticket> tickets, AuthInfo auth)
        {
            ...
            Log.Information(string.Format("Получена информация о {0} ШПИ", items.Count));
            Log.Information(string.Format("Из них некорректно обработано {0} ШПИ", items.Count(x => x.isCorrect == false)));
            Log.Information(string.Format("Из них не найдена информация о {0} ШПИ", items.Count(x => x.isExist == false)));
            Log.Information(string.Format("Корректно обработано {0} ШПИ", items.Count(x => x.isCorrect && x.isExist && x.isReady)));
        ...
        static void Main(string[] args)
        {
            ...
            Log.Logger = new LoggerConfiguration()
                ...
                .CreateLogger();
            Log.Information("Запуск службы");

            //Получение ШПИ из базы данных 
            List<Barcode> barcodes = new List<Barcode>() { new Barcode("sdf") };
            barcodes = GetBarcodes();
            //DebugFiles.WriteBarcodesToFile(barcodes, @"logs\Barcodes.txt");
            //barcodes = DebugFiles.ReadBarcodesFromFile(@"logs\Barcodes.txt");

            //Отправка ШПИ Почте России и получение билетов на получение информации
            List<Ticket> tickets;
            tickets = GetTickets(barcodes, admin);
            //DebugFiles.WriteTicketsToFile(tickets, @"logs\Tickets.txt");
            //tickets = DebugFiles.ReadTicketsFromFile(@"logs\Tickets.txt");
            ...
            //DebugFiles.WriteQueriesToFile(queries, @"logs\Queries.txt");
            //queries = DebugFiles.ReadQueriesFromFile(@"logs\Queries.txt");

            //Отправка запросов в базу данных
            DB.inputDataSQL(queries);
            Log.Information($"В базе данных обновлена информация о {queries.Count} ШПИ");
            Log.Information("Завершение работы службы");
