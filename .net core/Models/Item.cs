            ...
            if (Item.FirstChild.Name == "ns3:Error")
            {
                switch (Item.FirstChild.Attributes["ErrorTypeID"].Value)
                {
                    case "2":
                        ...
                        isCorrect = false;
                        Log.Error($"Формат данных запроса не соответствует протоколу {Barcode} | {Item.FirstChild.Attributes["ErrorName"].Value}");
                        break;
                    case "3":
                        //Неуспешная авторизация клиента при вызове метода
                        //throw new Exception(Item.FirstChild.Attributes["ErrorName"].Value);
                        isCorrect = false;
                        Log.Error($"Неуспешная авторизация клиента при вызове метода {Barcode} | {Item.FirstChild.Attributes["ErrorName"].Value}");
                        break;
                    case "6":
                    ...
                    case "12":
                        ...
                        //throw new Exception(Item.FirstChild.Attributes["ErrorName"].Value);
                        isExist = false;
                        Log.Error($"Информация об идентификаторе отправления отсутствует {Barcode} | {Item.FirstChild.Attributes["ErrorName"].Value}");
                        break;
                    case "16":
                        //Внутренняя ошибка работы Сервиса отслеживания
                        //throw new Exception(Item.FirstChild.Attributes["ErrorName"].Value);
                        isCorrect = false;
                        Log.Error($"Внутренняя ошибка работы Сервиса отслеживания {Barcode} | {Item.FirstChild.Attributes["ErrorName"].Value}");
                        break;
                    case "17":
                        //Время хранения ответа по билету истекло, ответ был удален с сервера
                        //throw new Exception(Item.FirstChild.Attributes["ErrorName"].Value);
                        isCorrect = false;
                        Log.Error($"Время хранения ответа по билету истекло, ответ был удален с сервера {Barcode} | {Item.FirstChild.Attributes["ErrorName"].Value}");
                        break;
                    default:
                        //throw new Exception();
                        isCorrect = false;
                        Log.Error($"Неизвестный номер ошибки {Barcode} | {Item.FirstChild.Attributes["ErrorName"].Value}");
                        break;
                }
                return;
            }
